...
| 🏷️ **智能标签提取** | 从 AI 回复中精准提取指定 XML 标签内容，并支持黑名单移除杂项标签 |
...
使用**标签提取**功能，可以只导出指定标签内的正文，也可以在预览和导出前移除不需要的标签块。
...
1. ✅ **勾选「原始 (chat.mes)」** — 强烈建议，读取未经正则处理的原始内容
2. 如果要白名单提取，模式选择 **「只提取指定标签」**
3. 填写标签名：`content` 或 `content summary`（多标签用空格/逗号分隔）
4. 如需剔除杂项，填写「移除标签」：`thinking note summary`
5. 如果只想做黑名单移除，可保持 **「全部内容」** 模式，只填写「移除标签」
6. 点击 **🔄 刷新预览** 确认提取效果
7. 导出时将应用相同规则！
...
| 输入方式 | 效果 |
|:---|:---|
| `thinking` | 移除 `<thinking>...</thinking>` 整段内容 |
| `thinking note` | 同时移除多个标签块 |
| `thinking, note, summary` | 逗号分隔也可以 |

> 黑名单会删除标签和其内部内容，建议搭配「原始 (chat.mes)」使用，也可与白名单提取同时使用。
...
1. ✅ 是否勾选了「原始 (chat.mes)」
2. 标签名是否正确（默认不区分大小写）
3. 如果启用了「移除标签」，确认没有把正文标签一并移除
4. 使用 `nagDebug()` 查看原始内容中是否包含该标签
5. 点击「🔄 刷新预览」查看提取效果
