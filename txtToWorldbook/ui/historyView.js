export function createHistoryView(deps = {}) {
    ...
    async function showHistoryView() {
        ...
        historyModal
            .querySelector('#ttw-clear-history')
            .addEventListener('click', async () => {
                const ok = await confirmAction('确定清空所有历史记录？', {
                    title: '清空历史记录',
                    danger: true,
                });
                if (!ok) return;
        ...
        historyModal.querySelectorAll('.ttw-history-item').forEach((item) => {
            item.addEventListener('click', async () => {
                ...
                historyModal.querySelectorAll('.ttw-history-item').forEach((node) => node.classList.remove('active'));
                ...
                <div style="margin-bottom:15px;padding-bottom:15px;border-bottom:1px solid #444;">
                    ...
                    <div style="margin-top:10px;">
                        <button class="ttw-btn ttw-btn-small ttw-btn-warning ttw-history-rollback-btn" data-history-id="${historyId}">
                            ⏪ 回退到此版本前
                        </button>
                    </div>
                </div>
                ...
                const rollbackBtn = detailContainer.querySelector('.ttw-history-rollback-btn');
                if (rollbackBtn && typeof onRollback === 'function') {
                    rollbackBtn.addEventListener('click', async () => {
                        await onRollback(parseInt(rollbackBtn.dataset.historyId, 10));
                    });
                }
            });
        });
